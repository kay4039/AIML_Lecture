...
# 제목과 설명
st.title("Iris Species Predicotr")
st.write("""
...
df = pd.DataFrame(X, columns=iris.feature_names) #sepal len, sepalwid, petal len, petal wid,
...
def user_input_features():
    ...
    petal_length = st.sidebar.slider("Petal_length (cm)",
                                    float(df['petal length (cm)'].min()),
                                    float(df['petal length (cm)'].max()),
                                    float(df['petal length (cm)'].mean()),
                                    )
    petal_width = st.sidebar.slider("Petal_width (cm)",
